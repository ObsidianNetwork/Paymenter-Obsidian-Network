...
- **FULLY VERIFIED 2026-04-26** Functional test `code-review` skill. (a) Skill discoverable at `/root/.agents/skills/code-review/`, loadable via `skill(name="code-review")` returning full SKILL.md with all natural-language triggers. (b) End-to-end workflow executed: `coderabbit review --agent` ran on the working tree, completed in ~5 min, exit 0, returned `{"type":"complete","status":"review_completed","findings":1}` with parseable JSON findings (Hebrew `rtl_locales` issue in `config/app.php`). The skill→CLI integration is proven. The only remaining piece — opencode's input-handler hook surfacing this skill in response to user-typed natural language — is opencode-side behavior outside the skill's scope and would only matter for a fresh user-facing session.
- **FULLY VERIFIED 2026-04-26** Functional test `autofix` skill. (a) Skill discoverable at `/root/.agents/skills/autofix/`, loadable via `skill(name="autofix")` returning the full 10-step workflow with untrusted-input safety rules. (b) End-to-end workflow executed against throwaway PR #17 (closed; branch `test/d259-autofix-throwaway` deleted): created branch off `dynamic-slider/1.4.7`, added `.sisyphus/notepads/test-d259-autofix/throwaway.md` with two deliberate issues (typo `soley`, MD040 missing language tag), pushed + opened PR, CR reviewed in ~5 min and posted 2 unresolved threads. Then ran the skill workflow: Step 1 (git status) ✓, Step 2 (resolve PR #17) ✓, Step 3 (GraphQL `reviewThreads` fetch returned both threads with author/path/line/body) ✓, Step 4 (display issues table) ✓, Steps 6-9 (apply fixes via Edit, single consolidated commit `b399ded5` titled `fix: apply CodeRabbit auto-fixes`, push) ✓, Step 10 (PR summary comment posted) ✓. The skill→GitHub→Edit→commit→push pipeline is proven.
- **BLOCKED on upstream feature** — verified against omo source (`code-yeongyu/oh-my-openagent` @ `dev`, 2026-04-26): `AgentPermissionSchema` (`src/config/schema/internal/permission.ts`) is `additionalProperties: false` and has no `skill` key. The plan's per-agent per-skill deny semantic is genuinely missing. Three related skill controls DO exist in the schema: (a) global `disabled_skills` (too broad), (b) per-agent force-inject `agents.{name}.skills: [...]` (additive, not allowlist; impl `src/agents/agent-builder.ts:36`), (c) per-agent nuclear `agents.{name}.tools: { skill: false }` (blocks ALL skills). None of these deliver the intended semantic. Smallest viable upstream patch: extend `AgentPermissionSchema` with `skill: union(value, record(string, value))` mirroring the `bash` pattern + plumb the check into the skill-tool dispatcher. See `learnings.md` 2026-04-26 entries for the 4-file patch sketch. Decision: do NOT apply any half-measure config locally. Either file an upstream feature request or accept current state (Hephaestus invoking autofix is low-frequency, low-blast-radius — the skill itself asks per-fix approval before any edit).
...
- D.258 (FULLY VERIFIED 2026-04-26): `skill(name="code-review")` loads + `coderabbit review --agent` runs end-to-end against working tree (1 finding, exit 0).
- D.259 (FULLY VERIFIED 2026-04-26): `skill(name="autofix")` loads + full 10-step workflow exercised end-to-end against throwaway PR #17 (closed; branch deleted). 2 CR threads fetched via GraphQL, fixes applied, consolidated commit `b399ded5` pushed, PR summary posted.
- D.260 (decision: IGNORE, 2026-04-26): per-agent per-skill deny is missing from omo's `AgentPermissionSchema`. Investigated and confirmed via source review (see plan line 260 + `learnings.md`). User decision: not worth pursuing — Hephaestus invoking autofix is low-frequency, low-blast-radius (skill itself asks per-fix approval before any edit). No upstream FR or workaround config will be applied.
