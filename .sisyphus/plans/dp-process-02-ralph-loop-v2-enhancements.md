...
- **PARTIALLY VERIFIED — skill machinery confirmed; natural-language hook still needs interactive session** Functional test `code-review` skill. Verified 2026-04-26 via `skill(name="code-review")` invocation: skill is discoverable at `/root/.agents/skills/code-review/`, loads cleanly, returns full SKILL.md with all natural-language triggers ("Review my code", "Run coderabbit", etc.). The remaining gap — whether opencode's input-handler hook detects those phrases in user input and triggers `skill({ name: "code-review" })` — is opencode-side behavior outside the skill itself, and genuinely needs a fresh user-facing opencode session to validate.
- **PARTIALLY VERIFIED — skill machinery confirmed; end-to-end flow needs throwaway PR + interactive session** Functional test `autofix` skill. Verified 2026-04-26 via `skill(name="autofix")` invocation: skill is discoverable at `/root/.agents/skills/autofix/`, loads cleanly, returns full 10-step workflow with untrusted-input safety rules intact. The remaining gap — invoking the skill on a real PR with unresolved CR threads, walking through `gh api graphql` thread fetch + `AskUserQuestion` per-fix approval + consolidated commit + push — needs (a) a throwaway PR with deliberate issues, (b) ~5 min wait for CR to review, (c) an interactive opencode session for the approval prompts.
...
- D.258 (partially verified): `skill(name="code-review")` loads cleanly with all natural-language triggers. Only opencode's input-handler hook test remains — needs interactive session.
- D.259 (partially verified): `skill(name="autofix")` loads cleanly. Only end-to-end PR-thread + approval flow remains — needs throwaway PR + interactive session.
- D.260 (blocked on upstream): per-agent per-skill deny missing from `AgentPermissionSchema`. The 3 related controls in current omo (global disable / per-agent force-inject / per-agent nuclear tool disable) all miss the intent. Path forward: file upstream feature request OR accept current state. See plan line 260 + `learnings.md` 2026-04-26 for the 4-file patch sketch.
