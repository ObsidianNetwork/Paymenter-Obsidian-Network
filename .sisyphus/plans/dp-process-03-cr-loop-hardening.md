...
- §Hard rules — add **Rule 8**:
  > **8. ≥10-minute quiet period after most recent CR activity before merge.** "CR activity" = any review submitted, thread comment, or `CodeRabbit` status change authored by `coderabbitai` since the last commit on the PR. The verify script computes `now - last_cr_activity_timestamp` and fails if < 600s. CR's auto-ack messages ("Actions performed: Review triggered.", "Actions performed: Comments resolved.") are excluded from "activity" via body-text filter.
  >
