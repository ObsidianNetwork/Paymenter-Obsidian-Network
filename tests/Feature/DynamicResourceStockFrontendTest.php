...
class DynamicResourceStockFrontendTest extends TestCase
{
    public function test_checkout_blocks_until_a_live_complete_vector_quote_succeeds(): void
    {
        ...
        $this->assertStringContainsString('retryQuote()', $view);
        $this->assertStringContainsString(
        ...
        $this->assertStringContainsString(
            'isCompleteResourceQuote(quote, this.expectedBoundIds)',
            $controller
        );
    }
    ...
    public function test_dynamic_upgrade_has_an_accessible_explicit_retry_path(): void
    {
        $view = file_get_contents(
            base_path('themes/default/views/services/upgrade.blade.php')
        );
        ...
        $this->assertStringContainsString(
            "\$root.querySelector('.dynamic-slider-input')?.focus()",
            $view
        );
        $this->assertStringContainsString(
    ...
    public function test_slider_uses_live_bound_for_pointer_keyboard_and_accessibility(): void
    {
        ...
        $this->assertStringContainsString(
            'keydown.end.prevent="applyKeyboardValue(max)"',
            $slider
        );
